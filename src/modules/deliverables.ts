...
import { updateProjectStatus, updateCurrentModule } from "../db/projects";
...
ВАЖНО: Тебе предоставлены утверждённые результаты всех этапов. НЕ пересоздавай их и НЕ предлагай альтернативы. Используй материалы КАК ЕСТЬ, только структурируй и оформи.
...
ВАЖНО: Тебе предоставлены утверждённые результаты всех этапов. НЕ пересоздавай их и НЕ предлагай альтернативы. Используй материалы КАК ЕСТЬ.`;
...
export async function handleFullReport(ctx: BotContext) {
  ...
  const materials = await collectMaterials(projectId);
  const reportText = await generateWithClaude(FULL_REPORT_PROMPT + "\n\n" + await getStyleGuide(), materials, {
    projectId,
...
export async function handleSummary(ctx: BotContext) {
  ...
  const materials = await collectMaterials(projectId);
  const summaryText = await generateWithClaude(SUMMARY_PROMPT + "\n\n" + await getStyleGuide(), materials, {
    projectId,
    moduleNum: MODULE_NUM,
    maxTokens: 1000,
  });
